...
def get_interacoes(a,b,n):
    ...
    interval = (b-a)/n
    interacoes = [a]
    for num in range(n): interacoes.append(round(interacoes[num]+interval,3))
    return interacoes
...
for x in x_i: 
    ...
    soma = soma + resultado
    if mostrar_calculos == True: print(f'\n ----------------- ITERACAO {contador-1} -----------------\n x = {round(x,3)} // f(x) = {round(execute_formula(form, x),3)} // f(x)*c = {round(resultado,3)}')
    
#Pega a soma final de todos os x_i e multipilica pelo tamanho das bases dos triangulos
resultado = (((b - a)/n)/3)*soma
print(f'\nRESULTADO FINAL: {round(resultado,3)}')
print(f'SOMA FINAL: {round(soma,3)}')
